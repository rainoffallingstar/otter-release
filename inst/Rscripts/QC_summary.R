...
QC_summary <- function(configfile){
  ...
  for (i in 1:length(SIDs)){
    ...
    mapping_ratio = bsmapStatT[grep("Mapping efficiency:",bsmapStatT)] %>%
      ...
      {.[[1]][2]} %>%
      stringr::str_trim()
    total_reads_pairs = bsmapStatT[grep("Sequence pairs analysed in total:",bsmapStatT)] %>%
      stringr::str_split(.,":") %>%
      {.[[1]][2]} %>%
      stringr::str_trim()
    aligned_reads_pairs = bsmapStatT[grep("Number of paired-end alignments with a unique best hit:",bsmapStatT)] %>%
      stringr::str_split(.,":") %>%
      {.[[1]][2]} %>%
      stringr::str_trim()
    ...
    qc_df_list[[i]] <- data.frame(
      ...
      GC_raw = GC_raw,
      GC_clean = GC_clean,
      BS_conversion_ratio = conversion_ratio,
