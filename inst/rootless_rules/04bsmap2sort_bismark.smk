rule bsmap2sort4homo:
  ...
  shell:
    """
    conda run -n bismark bismark --genome {params.genomeFile} --nucleotide_coverage --parallel {threads} -1 {input.trim_R1} -2 {input.trim_R2} -o {params.bsmapDir_species}  --temp_dir {params.bamTmp}
    
    conda run -n bismark samtools sort -@ {threads} -o {params.bam_sorted} {params.bam_aligned}
    
    conda run -n bismark samtools index -@ {threads} -b {params.bam_sorted}
