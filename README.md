...
The script downloads pre-built binaries from GitHub Releases and sets up the required conda environments interactively. The first interactive prompt lets you choose English or Chinese. Use `--lang en` or `--lang zh` to force the interface language, or use `scripts/setup.sh` only for source builds. When existing binaries are detected, the installer asks once whether to overwrite them, then applies that decision to all remaining binaries in the same run.
