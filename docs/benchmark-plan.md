...
The first bounded build is `mm10-canary@GRCm38-gencode-M25-chr19`: Ensembl GRCm38 chromosome `19` FASTA with a GENCODE M25 GTF filtered to contig `19`, plus Bismark/Bowtie2/STAR indexes. Ensembl release 100 publishes a BSD `sum` provider checksum for this 17 MB FASTA, which is verified explicitly; the GENCODE GTF retains its provider MD5 verification. This is deliberately a technical canary for Craftmake/Slurm/index/publication/recovery validation; it must never be reported as a full `mm10`/`mm38` result or used for scientific parity. A successful canary de-risks the larger `mm10`/`mm38` release but does not satisfy its acceptance criteria.
