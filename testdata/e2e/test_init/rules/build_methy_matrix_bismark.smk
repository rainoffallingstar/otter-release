rule bismark_methylation_extractor :
  ...
  params:
    ...
    genomeFile = config["genomeFile"][config["species"].index(config["workflow"]["species"]["graft"])],
  ...
  shell:
    """
      enva run bismark -- samtools sort  -@ {threads} -n -o {params.bam_nsorted} {input.bam_sorted}
      
      # repair unpaired reads
      enva run bismark -- samtools view {params.bam_nsorted}  | awk '{{print $1}}' | sort > {params.bam_readnames}
      ...
      enva run bismark -- bismark_methylation_extractor --paired-end --gzip \
