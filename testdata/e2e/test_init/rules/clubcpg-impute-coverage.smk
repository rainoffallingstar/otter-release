rule clubcpgimputecoverage:
  ...
  output:
    os.path.join(config["clubcpg_coverage_impute"], "CompleteBins."+"{sample}_"+config["workflow"]["species"]["graft"]+".bam."+"{chr}.IMPUTED_filter.csv")
  params:
    ...
    filter_output = lambda wildcards:os.path.join(config["clubcpg_coverage_impute"], "CompleteBins."+f"{wildcards.sample}_"+config["workflow"]["species"]["graft"]+".bam."+f"{wildcards.chr}.IMPUTED_filter.csv"),
    origin_output = lambda wildcards:os.path.join(config["clubcpg_coverage_impute"], "CompleteBins."+f"{wildcards.sample}_"+config["fixed"]+config["workflow"]["species"]["graft"]+".bam."+f"{wildcards.chr}.csv.IMPUTED.csv")
  threads:24
  shell:
    """
    mkdir {params.models}
