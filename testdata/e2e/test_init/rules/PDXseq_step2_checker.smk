rule rnaseq_step2_checker :
  ...
  input:
    ...
    expand(os.path.join(config["directories"]["qc"]["main"],"GCbias","{sample}_{species}", "gc_bias_metrics.txt"),sample= config["SIDs"],species = config["species"]),
    expand(os.path.join(config["directories"]["qc"]["main"],"GCbias","{sample}_{species}", "gc_bias_metrics.pdf"),sample= config["SIDs"],species = config["species"]),
    expand(os.path.join(config["directories"]["qc"]["main"],"GCbias","{sample}_{species}", "summary_metrics.txt"),sample= config["SIDs"],species = config["species"]),
    ...
    expand(os.path.join(config["directories"]["bsmap"]["main"],"Filtered_bams" ,"{sample}_fixed_"+config["workflow"]["species"]["graft"]+"_Filtered.bam"),sample=config["SIDs"]),
